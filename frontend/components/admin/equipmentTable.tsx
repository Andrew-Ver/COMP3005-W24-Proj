...
export default function EquipmentTable() {
    ...
    const fetchRoomIds = async () => {
        try {
            const response = await fetch(`/api/room/get`, {
                ...
                body: JSON.stringify({ user : 6 }), // Include username in request body
            });
    ...
    return (
        <Stack gap="sm" align="center">
            ...
            <ExampleWithProviders roomIds={roomIds}/>
...
const Example = ({ roomIds }: ExampleProps) => {
    ...
    const columns = useMemo<MRT_ColumnDef<Metric>[]>(
        () => [
            ...
            },
            {
                ...
                accessorFn: (row) => { return row.needs_maintenance ? 'Yes' : 'No' },
                enableEditing: false
            },
            {
    ...
    const table = useMantineReactTable({
        ...
        renderTopToolbarCustomActions: ({ table }) => {
            ...
            return (
                ...
                >
                    Mark as Maintained
                </Button><Tooltip label="Delete">
                    <ActionIcon color="red" onClick={()=>openDeleteConfirmModal(table.getSelectedRowModel().rows[0])}>
                        <IconTrash />
                    </ActionIcon>
                </Tooltip></>
            );
    ...
    const handleMarkAsNeedsMaintenance = async () => {
        ...
        const equipment_id: number[] = selectedRows.map(row => row.original.equipment_id);
        try {
...
function validateMetric(equipment: Metric) {
    return {
...
const ExampleWithProviders = ({ roomIds }: ExampleProps) => (
