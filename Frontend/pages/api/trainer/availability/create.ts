...
export default async function addTrainerAvailability(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        const conflictCheckResult = await pool.query(conflictCheckQuery, [username, begin_time, end_time]);
        ...
        const insertResult = await pool.query(insertQuery, [username, false, begin_time, end_time]);
