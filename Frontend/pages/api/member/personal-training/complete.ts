...
export default async function markSessionsComplete(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        if (session_ids && session_ids.length > 0) {
            ...
            res.status(200).json({ message: 'Sessions marked as complete successfully' });
        } else {
