...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const { availability_id, begin_time, end_time, rate_per_hour, description, member_username } = sessionResult.rows[0];
        ...
        const insertBillQuery = `
            ...
            RETURNING *;
        `;
        const refundResult = await pool.query(insertBillQuery, [member_username, amount, description]);
